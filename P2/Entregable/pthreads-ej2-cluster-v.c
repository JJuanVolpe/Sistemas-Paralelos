...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("Uso: %s <Tamaño de matriz N> <Tamaño de bloque bs> <Número de hilos>\n", argv[0]);
        return 1;
    }

    int N = atoi(argv[1]);  // Obtengo tamaño de las matrices
    int bs = atoi(argv[2]);  // // Obtengo tamaño del bloque 
    int num_threads = atoi(argv[3]);

    if (N % bs != 0) {
        printf("Error: N debe ser divisible por bs\n");
        return 1;
    }
